...
# Part3: How long have the volcanic eruptions continued ?
...
eruption_data %>%
  ggplot() +  
  geom_point(aes(x = year(startdate_erupt),y = eruption_duration,
                 color=volcano_name, fill=volcano_name,
                 size = eruption_duration,alpha=vei)) +
  scale_size(range = c(1,20))+
  scale_fill_brewer(palette = "RdYlBu",guide = "legend")+
...
p <- eruptions %>%
  ...
  mutate(
    ...
    mutate(eruption_duration = round(as.numeric(
      difftime(enddate_erupt, startdate_erupt, unit = "days")), 0)) %>% 
  mutate(s2_rolling = slide_dbl(startdate_erupt, sum, .before = 1, .complete = TRUE))
  
  mutate(prev = lag(year(startdate_erupt), 1)) %>% 
  mutate(MR_bar = abs(Average_xbar - prev))


p <- ggplot(
  eruption_data, 
  aes(year(startdate_erupt),y = eruption_duration, 
      size = eruption_duration, colour = volcano_name)
  ) +
...
p + transition_time(startdate_erupt) +
